...
(async function run() {
  // --- Erfolgsfall: Status, Dauer, JSON-Struktur und Body-Auszug werden gemeldet ---
  {
    ...
    console.log("Block 1/6 (Erfolgsfall: Status/JSON-Struktur/Dauer): OK");
  }
  ...
  // --- 400 mit Fehlertext: der Koerper ist die eigentliche Information ---
  {
    ...
    console.log("Block 2/6 (4xx: Fehlertext des Servers bleibt erhalten): OK");
  }
  ...
  // --- Zeitueberschreitung: klare Meldung statt Haenger ---
  {
    ...
    console.log("Block 3/6 (Zeitueberschreitung sauber gemeldet, " + (dt / 1000).toFixed(1) + "s): OK");
  }
  ...
  // --- Netzwerkfehler: .cause enthaelt bei fetch oft erst den echten Grund ---
  {
    ...
    console.log("Block 4/6 (Netzwerkfehler: .cause wird mitgemeldet): OK");
  }
  ...
  // --- Kaputtes JSON trotz JSON-Content-Type darf die Diagnose nicht abschiessen ---
  {
    ...
    console.log("Block 5/6 (HTML statt JSON: Bot-Sperre wird sichtbar): OK");
  }
  ...
  // --- Handler: Gesamtantwort ist gueltiges JSON mit Zusammenfassung, auch wenn alles scheitert ---
  {
    ...
    assert.strictEqual(body.proben.length, 3, "only=reliefweb muss genau die drei ReliefWeb-Proben auswaehlen");
    assert.strictEqual(body.zusammenfassung.fehlgeschlagen.length, 3);
    ...
    console.log("Block 6/6 (Handler: robust + keine Geheimnisse im Bericht): OK");
  }
