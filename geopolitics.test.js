...
(async function run() {
  // --- computeLevel(): Stufenlogik isoliert pruefen ---
  {
    ...
    console.log("Block 1/13 (Stufenlogik): OK");
  }
  ...
  // --- Normalfall: ReliefWeb liefert 2 Laender, GDELT antwortet fuer alle ---
  {
    ...
    console.log("Block 2/13 (Normalfall, kombinierte Signale): OK");
  }
  ...
  // --- ReliefWeb faellt aus -> trotzdem Bericht mit fester Watchlist, Fehler sichtbar ---
  {
    ...
    console.log("Block 3/13 (ReliefWeb-Ausfall, feste Liste als Basis): OK");
  }
  ...
  // --- Zeitbudget: haengende GDELT-Aufrufe duerfen den Bericht nicht blockieren ---
  {
    ...
    console.log("Block 4/13 (Zeitbudget schuetzt vor Timeout-Sturm): OK");
  }
  ...
  // --- ReliefWeb: 400 auf gefilterte Anfrage -> Fallback auf ungefilterte Anfrage ---
  {
    ...
    console.log("Block 5/13 (ReliefWeb 400 -> Fallback ohne Statusfilter): OK");
  }
  ...
  // --- Zeitbudget gilt AB FUNKTIONSSTART, nicht erst nach ReliefWeb ---
  {
    ...
    console.log("Block 6/13 (Zeitbudget ab Funktionsstart, deckt ReliefWeb + GDELT zusammen ab): OK");
  }
  ...
  // --- ReliefWeb: Statuswerte + Fehlertext des Servers ---
  {
    ...
    console.log("Block 7/13 (ReliefWeb: korrekte Statuswerte + Fehlertext durchgereicht): OK");
  }
  ...
  // --- Timeout darf NICHT auf die ungefilterte Abfrage nachfassen (Zeitverschwendung) ---
  {
    ...
    console.log("Block 8/13 (ReliefWeb: kein sinnloser zweiter Versuch nach Timeout): OK");
  }
  ...
  // --- API-Version: v1 ist abgeschaltet, es darf nur noch v2 angefragt werden ---
  {
    ...
    console.log("Block 9/13 (nur API v2 wird angefragt): OK");
  }
  ...
  // --- 410 darf NICHT zu weiteren Versuchen fuehren (Version endgueltig weg) ---
  {
    ...
    console.log("Block 10/13 (410 beendet die Kette sofort): OK");
  }
  ...
  // --- Flache v2-Antwort (ohne fields-Umschlag) muss ebenfalls verarbeitet werden ---
  {
    ...
    console.log("Block 11/13 (flaches v2-Format wird ebenfalls verstanden): OK");
  }
  ...
  // --- Unbekannter Antwortumschlag: Fehler statt stiller leerer Liste ---
  {
    ...
    console.log("Block 12/13 (unbekanntes Antwortformat wird gemeldet, nicht verschluckt): OK");
  }

  // --- Live-Befund: 'country.iso2' wird von ReliefWeb v2 mit HTTP 400 abgelehnt
  //     ("Unrecognized field 'country.iso2' in parameter 'fields'") - darf nicht
  //     mehr angefragt werden. Appname muss ueber RELIEFWEB_APPNAME konfigurierbar
  //     sein (Live-Befund: v2 verlangt einen REGISTRIERTEN Appnamen, HTTP 403
  //     "not using an approved appname" - das laesst sich nur per Registrierung
  //     und Umgebungsvariable loesen, nicht durch eine andere Anfrageform). ---
  {
    const seen = [];
    global.fetch = async (url) => {
      const u = String(url);
    ...
    let { buildReport } = require(path)._internal;
    await buildReport();
    ...
    assert.ok(seen.every((u) => u.includes("appname=terminal-app-geopolitics")),
      "ohne RELIEFWEB_APPNAME-Umgebungsvariable muss der bisherige Standard-Appname verwendet werden");

    seen.length = 0;
    process.env.RELIEFWEB_APPNAME = "mein-genehmigter-appname";
    delete require.cache[require.resolve(path)];
    ({ buildReport } = require(path)._internal);
    await buildReport();
    delete process.env.RELIEFWEB_APPNAME;
    assert.ok(seen.length > 0 && seen.every((u) => u.includes("appname=mein-genehmigter-appname")),
      "ein per Umgebungsvariable gesetzter (genehmigter) Appname muss verwendet werden, damit ReliefWeb ohne Code-Aenderung nutzbar wird, sobald einer registriert ist");
    console.log("Block 13/13 (country.iso2 entfernt, Appname per Umgebungsvariable konfigurierbar): OK");
  }
